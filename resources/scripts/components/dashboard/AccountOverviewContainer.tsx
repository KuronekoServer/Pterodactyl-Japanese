    ...
    return (
        <PageContentBlock title={t('account_overview')}>
            ...
            <Container css={[tw`lg:grid lg:grid-cols-3 mb-10`, state?.twoFactorRedirect ? tw`mt-4` : tw`mt-10`]}>
                ...
                <ContentBox css={tw`mt-8 sm:mt-0 sm:ml-8`} title={t('update_email_address')} showFlashes={'account:email'}>
                    <UpdateEmailAddressForm />
                </ContentBox>
                <ContentBox css={tw`md:ml-8 mt-8 md:mt-0`} title={t('two_factor_authentication')}>
                    <ConfigureTwoFactorForm />
                </ContentBox>
                <ContentBox css={tw`mt-8`} title={t('display_language')} showFlashes={'account:language'}>
                    <UpdateLanguageForm />
                </ContentBox>
            </Container>
        </PageContentBlock>
